...
Убедилcя, что **полный сетевой доступ пропал**
- Правило по умолчанию запрещает подключения, не разрешённые специально.
- Среди разрешённых подключений нет подключения `frontend` -> `cache`, отсюда ошибка соединения.
Вывод - **сетевые политики работают**.
